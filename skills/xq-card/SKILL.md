...
```bash
node ~/.claude/skills/xq-card/assets/capture.js <html> <png> <width> <height> [fullpage]
```

依赖：`~/.claude/skills/xq-card/node_modules/` 中的 playwright。如报错：

```bash
cd ~/.claude/skills/xq-card && npm install playwright && npx playwright install chromium
```
