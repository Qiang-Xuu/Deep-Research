...
本 skill 面向工程师群体，所有模具统一遵循"技术博客风"：

- **信息密度优先**：每张卡片必须承载足够的信息量，禁止大面积无意义留白
- **代码与架构友好**：保留代码块、命令行、架构框图的清晰呈现能力
- **配色克制**：优先深色主题（类似技术文档暗色模式）或极简白底黑字，禁花哨渐变、霓虹光晕
- **字体偏 monospace/等宽**：技术术语、代码片段、性能数据必须用等宽字体呈现
- **数据可视化**：性能对比、架构层级、吞吐量/延迟数据优先用图表或结构化表格呈现
- **去文艺化**：不追求"设计感"，追求"一看就懂、一扫即得"的工程效率
