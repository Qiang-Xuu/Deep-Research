---
name: xq-paper
description: "Paper reader for engineers. Takes a paper and extracts its ideas for personal use. Focuses on understanding the author's thought process, not just conclusions. Use when user shares an arxiv link, paper URL, PDF, or asks to analyze a research paper. Trigger words: '读论文', '分析论文', 'paper', or when user shares an academic paper. Especially tuned for autonomous driving, high-performance computing, distributed systems, PCIe, virtualization, and inference optimization papers."
user_invocable: true
version: "4.4.0"
---
...
10. *数据检验* — 论文给出性能数据时，必须标注测试条件（硬件、数据集规模、baseline 配置）和可复现性。不标注条件的数字是噪音
...
*开头先立锚点*：找到一个具象的中心隐喻或画面，在翻译的第一段就亮出来。优先从工程直觉中找锚点——"这就像 PCIe 的带宽争抢""相当于缓存失效的放大版"。后面所有概念围绕这个锚点生长，不是并列罗列。
...
落点在"能学"，不在"能用"。不仅问"这能不能用到我的系统里"，更问"这个思路的基因能嫁接到我系统的哪一块？嫁接需要什么条件？"

用四个视角试探连接，命中展开，没命中跳过，全没命中说「没有」：

- *迁移*：论文的某个机制/视角能移植升级我体系的某个零件吗？具体怎么接？需要什么前提条件？
- *混搭*：论文的某个组件和我已有的东西组合能产生新东西吗？产出什么？边界在哪？
- *反转*：论文的做法和我的默认假设相反吗？该停下什么、开始什么？
- *方法论*：作者解决这个问题的思路本身能不能复用？逼出这个解法的真问题是什么？之前的人卡在哪一步？作者的突破点是什么？这个思路放在我的领域会怎么变形？
